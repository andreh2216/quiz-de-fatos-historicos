...
const nome = inputData.question("Digite o seu nome: ");
...
function selecionarPerguntas(colecao, quantidade){

  const questoesEmbaralhadas = embaralharArray(colecao);
  return questoesEmbaralhadas.slice(0, quantidade);

}

function exibirPerguntas(colecao, posicao){
  
  var pergunta = selecionarPerguntas(colecao);

  var objeto =  pergunta[posicao];

  console.log(objeto.pergunta);
...
const perguntaSelecionada = selecionarPerguntas(questoes, 10);
...
while(contador <= 9){
  ...
  const exibirPergunta = exibirPerguntas(perguntaSelecionada,contador);

  let resposta = inputData.question("\nDigita sua Resposta: ");




  if(perguntaSelecionada[contador].resposta == resposta){
...
console.log("Pontuação final: "+acertos+"/"+erros+" acertos.");
